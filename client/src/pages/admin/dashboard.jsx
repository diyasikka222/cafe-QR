...
import AdminNavbar from '../../components/admin/adminNavbar.jsx'; // Ensure this path is correct
import { CheckCircle2, Clock, IndianRupee, Users, ChefHat } from 'lucide-react';

const AdminDashboard = () => {
    // 1. STATE: Orders & Analytics
    const [orders, setOrders] = useState([
        {
            ...
            total: 205,
            time: "07:45 PM",
            status: "Preparing" // Statuses: New, Preparing, Served
        },
        {
            id: "#ORD-7722",
            table: "12",
    ...
    const updateStatus = (id) => {
        setOrders(prevOrders => prevOrders.map(order => {
            ...
            return order;
        }).filter(order => order.status !== "Served")); // Remove once served/completed
    };
        ...
        <div className="min-h-screen bg-[#0f1115] text-slate-200 font-sans">
            ...
            <main className="p-4 md:p-8 max-w-[1600px] mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    <StatCard icon={<IndianRupee className="text-emerald-500" />} label="Revenue Today" value="₹12,450" />
                    <StatCard icon={<ChefHat className="text-orange-500" />} label="Active Orders" value={orders.length} />
                    <StatCard icon={<Clock className="text-blue-500" />} label="Avg. Prep Time" value="14m" />
                    <StatCard icon={<Users className="text-purple-500" />} label="Tables Active" value="8/20" />
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                    ...
                    <div className="flex items-center gap-4 bg-[#1a1d23] py-2 px-6 rounded-2xl border border-slate-800 shadow-2xl">
                        ...
                        <span className="text-xs font-black tracking-[0.2em] uppercase text-slate-300">Live Server</span>
                    </div>
                </div>
                ...
                {orders.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {orders.map((order) => (
                            <OrderCard key={order.id} order={order} onComplete={() => updateStatus(order.id)} />
                        ))}
                    </div>
                ) : (
                    <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-slate-800 rounded-[3rem] bg-[#16191e]/50">
                        ...
                        <p className="text-slate-500 font-bold text-xl uppercase tracking-widest">All orders served</p>
                    </div>
                )}
            </main>
        </div>
    ...
    <div className="group relative bg-[#1a1d23] rounded-[2.5rem] overflow-hidden border border-slate-800 hover:border-orange-500/40 transition-all duration-500 shadow-2xl">
        ...
        <div className="p-8">
            <div className="flex justify-between items-start mb-8">
                <div>
                    <span className="text-[10px] font-bold text-orange-500 uppercase tracking-[0.2em]">Table Number</span>
                    <h2 className="text-5xl font-black text-white leading-tight">{order.table}</h2>
                </div>
                <div className="text-right">
                    <p className="text-[10px] font-mono text-slate-500 mb-2 uppercase">{order.id}</p>
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                        order.status === "New" ? "bg-blue-500/10 text-blue-400" : "bg-orange-500/10 text-orange-400"
                    }`}>
                        ● {order.status}
                    </span>
                </div>
            ...
            <div className="pt-8 border-t border-slate-800/50 flex justify-between items-center">
                <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Total Bill</p>
